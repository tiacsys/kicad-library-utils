...
parser = argparse.ArgumentParser(
    description="Compare two .lib files to determine which symbols have changed"
)

parser.add_argument(
    "--new", help="New (updated) .lib file(s), or folder of .lib files", nargs="+"
)
parser.add_argument(
    "--old",
    help="Old (original) .lib file(s), or folder of .lib files for comparison",
    nargs="+",
)
parser.add_argument(
...
for lib_name in new_libs:
    ...
    for cmp in new_cmp:
        ...
        if new_cmp[cmp]["alias_of"] != old_cmp[cmp]["alias_of"] and args.verbose:
            printer.white(
                "Changed alias state of '{lib}:{name}'".format(lib=lib_name, name=cmp)
            )
        ...
        if not chk_old == chk_new:
            ...
            if args.design_breaking_changes:
                ...
                for pin_old in old_cmp[cmp]["cmp"].pins:
                    ...
                    if (
                        pin_old["posx"] != pin_new["posx"]
                        or pin_old["posy"] != pin_new["posy"]
                    ):
                        if (
                            pin_old["electrical_type"] == "N"
                            and pin_new["electrical_type"] == "N"
                        ):
                            nc_pins_moved += 1
                        else:
                            pins_moved += 1
