...
parser = argparse.ArgumentParser(
    description="Find duplicate parts (including aliases) in a .lib file"
)
    ...
    for cmp in lib.components:
        ...
        if cmp.name in unique_names.keys():
            ...
            for key in unique_names.keys():
                ...
                if cmp.name in aliases:
                    printer.yellow(
                        "Component '{cmp}' exists as an alias of '{key}'".format(
                            cmp=cmp.name, key=key
                        )
                    )
                    errors += 1
        ...
        for alias in cmp.aliases.keys():
            ...
            if alias in unique_names:
                printer.yellow(
                    "Component '{cmp}' ALIAS '{alias}' already exists".format(
                        cmp=cmp.name, alias=alias
                    )
                )
                errors += 1
