...
class CheckComponent:
    ...
    def resize_field(self, field):
        self.print_header()
        print("\t\t[resize] field size: %s -> %i" % (field["text_size"], 50))

        field["text_size"] = "50"

    def resize_pin_name_text(self, pin):
        self.print_header()
        print("\t\t[resize] pin name text size: %s -> %i" % (pin["name_text_size"], 50))

        pin["name_text_size"] = "50"

    def resize_pin_num_text(self, pin):
        self.print_header()
        print("\t\t[resize] pin num text size: %s -> %i" % (pin["num_text_size"], 50))
...
parser.add_argument(
    "-y",
    "--apply",
    help="Apply the suggested modifications in the report",
    action="store_true",
)
parser.add_argument(
    "-v",
    "--verbose",
    help="Print output for all pins - violating or not",
    action="store_true",
)
