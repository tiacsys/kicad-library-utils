...
class Rule(KLCRule):
    ...
    def stackStr(self, stack):
        ...
        return "Pinstack @ ({x},{y}){u}".format(
            x=int(stack["x"]), y=-1 * int(stack["y"]), u=unit_str
        )
            ...
            if len(loc["pins"]) > 1:
                ...
                for pin in loc["pins"]:
                    ...
                    if pin["electrical_type"] == "N":
                        self.error(
                            "NC {pin} @ ({x},{y})is stacked on other pins".format(
                                pin=self.pinStr(pin),
                                x=pin["posx"],
                                y=-1 * int(pin["posy"]),
                            )
                ...
                if len(pin_nums) < len(loc["pins"]):
                    self.error(
                        "Duplicate pins @ ({x},{y})".format(
                            x=loc["x"], y=-1 * int(loc["y"])
                        )
                    )
                    err = True
                    for pin in loc["pins"]:
                ...
                isSpecialSingleTypeStack = (len(pin_etypes) == 1) and (
                    ("w" in pin_etypes) or ("O" in pin_etypes)
                )
                if (len(pin_etypes) > 1) or isSpecialSingleTypeStack:
                    ...
                    if not isSpecialXPassivePinStack:
                        ...
                        for pin in loc["pins"]:
                            ...
                            if (
                                pin["electrical_type"] == "O"
                                or pin["electrical_type"] == "w"
                                or pin["electrical_type"] == "W"
                            ):
                ...
                while i < len(self.component.drawOrdered):
                    ...
                    if all([p_test[key] == pin[key] for key in keys]):
                        ...
                        if count > 1:
                            ...
                            self.info(
                                "Deleting {pin} @ ({x},{y})".format(
                                    pin=self.pinStr(pin), x=pin["posx"], y=pin["posy"]
                                )
                            )
                            continue
                    i += 1
        ...
        for pin in self.component.pins:
            if pin["num"] in self.fix_make_passive:
                ...
                self.info(
                    ...
                    + " "
                    + pin["name"]
                    + " is passive now (pin['electrical_type']="
                    + pin["electrical_type"]
                    + ")"
                )
