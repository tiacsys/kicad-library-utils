...
class Rule(KLCRule):
    ...
    def check(self) -> bool:
        ...
        if self.component.is_power_symbol() or self.component.is_graphic_symbol() or self.component.extends is not None:
            return False

        # check if component has just one rectangle, if not, skip checking
        self.center_rect_polyline = self.component.get_center_rectangle(range(self.component.unit_count))
        ...
        if self.component.is_small_component_heuristics():
            if (not math.isclose(self.center_rect_polyline.stroke_width, mil_to_mm(10))):
                self.warning("Component outline is thickness {0}mil, recommended is {1}mil for standard symbol".format(mm_to_mil(self.center_rect_polyline.stroke_width), 10))
                self.warningExtra("exceptions are allowed for small symbols like resistor, transistor, ...")
                rectangle_need_fix = False
        else:
            if (not math.isclose(self.center_rect_polyline.stroke_width, mil_to_mm(10))):
                self.error("Component outline is thickness {0}mil, recommended is {1}mil".format(mm_to_mil(self.center_rect_polyline.stroke_width), 10))
                rectangle_need_fix = True

        if (self.center_rect_polyline.fill_type != 'background'):
            msg = "Component background is filled with {0} color, recommended is filling with {1} color".format(self.center_rect_polyline.fill_type, 'background')
            if self.component.is_small_component_heuristics():
                self.warning(msg)
                self.warningExtra("exceptions are allowed for small symbols like resistor, transistor, ...")
            else:
