...
@click.option('--output-dir', '-o', type=click.Path(file_okay=False, dir_okay=True,
              ...
              default='stm32_datasheets', help='Output directory')
...
def download_datasheets(cubemx_mcu_db_dir, part_number_pattern, output_dir):
    ...
    part_numbers = {pn for pn in part_numbers
                    if fnmatch.fnmatch(pn.lower(), part_number_pattern.lower())}
    already_downloaded = {pn for pn in part_numbers if (output_dir / f'{pn}.pdf').is_file()}
    print(f'Found {len(part_numbers)} parts, of which {len(already_downloaded)} '
    ...
    with multiprocessing.Pool(50) as pool:
        ...
        for pn in part_numbers:
            pool.apply_async(fetch, (pn,), callback=save_pdf, error_callback=error).wait()
        pool.close()
        pool.join()
    progress.close()
